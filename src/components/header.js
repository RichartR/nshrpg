...
const URL = "https://xcaoefgporrjzlirttaz.supabase.co/rest/v1/";
...
// --------------------------------------------------------------------------------
// FUNCIONES DE RENDERIZADO ASÍNCRONO
// --------------------------------------------------------------------------------

async function renderLimitedTech(category, subcategory) {
    const techniques = await obtainLimitedTech(null, category, subcategory);

    if (!techniques || techniques.length === 0) {
        return '<li class="dropdown-item disabled"><span>No hay técnicas limitadas</span></li>';
    }

    return techniques.map(tech => `
        <li class="dropdown-item">
            <a href="#tech/${tech.id}" class="nav-link">
                ${tech.technique_name}
            </a>
        </li>
    `).join('');
...
async function renderFinalDropdown(affiliationItem, category, subcat, level) {
    const techCategory = category;
    const techSubcategory = subcat;

    const generalTechLink = subcat
        ? `<a href="#${affiliationItem}/${category}/${subcat}/tecnicas-generales" class="nav-link">Generales ${subcat}</a>`
        : `<a href="#${affiliationItem}/${category}/tecnicas-generales" class="nav-link">Generales ${category}</a>`;

    const limitedTechHTML = await renderLimitedTech(techCategory, techSubcategory);
...
async function renderHeader() {
    ...
    const affiliationItems = await Promise.all(
        affiliations.map(async (affiliationItem) => {
                    ...
                    if (subcats.length === 0) {
                        return `
                            <li class="dropdown-item has-submenu" data-category="${category}" data-menu-id="submenu-${category}">
                                ...
                                ${await renderFinalDropdown(affiliationItem, category, undefined, 3)}
                            </li>
                        `;
                    }
                        ...
                        subcats.map(async (subcat) => {
                            return `
                                <li class="dropdown-item has-submenu" data-subcategory="${subcat}" data-menu-id="submenu-${subcat}">
                                    ...
                                    ${await renderFinalDropdown(affiliationItem, category, subcat, 4)}
                                </li>
                            `;
                        })
                    );
            ...
            const categoriesHTML = categories.length > 0 ? categoryItems.join("") : '<li class="dropdown-item disabled"><span>No hay categorías</span></li>';
...
// --------------------------------------------------------------------------------
// FUNCIONES DE DATOS (ASÍNCRONAS Y SÍNCRONAS)
// --------------------------------------------------------------------------------
...
async function obtainLimitedTech(data, category, subcategory) {
    try {
        let body = `technique_details?availability_type=neq.unlimited&category_name=ilike.${category}`;
        if (subcategory !== undefined && subcategory !== null) {
            body += `&subcategory_name=ilike.${subcategory}`;
        }
        ...
        if (!response.ok) throw new Error("Error fetching limited techniques");
        const result = await response.json();

        console.log(result)
        return result;
    } catch (error) {
        console.error("Error al obtener los datos limitados:", error);
        return [];
    }
}